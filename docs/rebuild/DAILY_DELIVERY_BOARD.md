...
| 2026-07-18 (pending) | Integration | B → D → C into `integration/rebuild-nightly` | _in progress_ | _pending_ | _pending_ | Merge commits only, no directory overwrite. This row will be finalized once the integration branch's own typecheck/test run completes. |
