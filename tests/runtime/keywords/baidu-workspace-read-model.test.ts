...
describe("百度关键词工作台读取模型", () => {
  it("始终以客户和项目双重范围读取，并汇总真实记录", async () => {
    ...
    const db = { query: async <T>(sql: string, params: readonly SqlParam[] = []): Promise<DbQueryResult<T>> => {
      ...
      if (sql.includes("keyword_reference_source_import")) return { rows: [{
      ...
      return { rows: [
        { id: "raw-1", import_id: "import-a", raw_keyword: "GEO 服务", normalized_keyword: "geo 服务", seed_keyword: "GEO", metric_value: "19", evidence_status: "OBSERVED_DEMAND", observed_at: "2026-07-19T12:00:00.000Z" },
        { id: "raw-2", import_id: "import-a", raw_keyword: "GEO 公司", normalized_keyword: "geo 公司", seed_keyword: "GEO", metric_value: null, evidence_status: null, observed_at: "2026-07-19T12:00:00.000Z" },
      ] as T[], rowCount: 2 };
    }};
    ...
    expect(model.totals).toEqual({ imports: 1, keywords: 2, withObservedDemand: 1, rejectedRows: 1 });
