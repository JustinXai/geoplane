...
describe("validateMigrationManifest", () => {
  it("passes for the real migrations/manifest.json (matches the directory)", () => {
    ...
    expect(highestRequiredMigrationVersion(realMigrationsDir)).toBe("0018");
  });
...
describe("evaluateMigrationReadiness (floor semantics)", () => {
  ...
  it("PASSes when all required versions (0001-0018) are applied", () => {
    const r = evaluateMigrationReadiness(all, realMigrationsDir);
    expect(r.status).toBe("PASS");
    expect(r.databaseAheadOfBuild).toBe(false);
    expect(r.highestRequired).toBe("0018");
  });

  it("FAILs when the DB is missing 0018", () => {
    const r = evaluateMigrationReadiness(new Set([...all].filter((version) => version !== "0018")), realMigrationsDir);
    expect(r.status).toBe("FAIL");
    expect(r.missing).toContain("0018");
  });
  ...
  it("PASSes with DATABASE_AHEAD_OF_BUILD when the DB has 0001-0019", () => {
    const ahead = new Set([...all, "0019"]);
    const r = evaluateMigrationReadiness(ahead, realMigrationsDir);
    expect(r.status).toBe("PASS");
    expect(r.databaseAheadOfBuild).toBe(true);
    expect(r.ahead).toEqual(["0019"]);
  });
});
