...
export class VerticalPolicyPackRegistry implements VerticalPolicyPackPort {
  ...
  register(pack:VerticalPolicyPackDefinition):void{const key=this.key(pack.packId,pack.version);if(this.versions.has(key))throw new DuplicatePolicyPackError(`Policy pack ${key} is already registered.`);this.versions.set(key,Object.freeze({...pack,rules:Object.freeze(pack.rules.map(r=>Object.freeze({...r,evidenceReferences:Object.freeze([...r.evidenceReferences])}))),evidenceReferences:Object.freeze([...pack.evidenceReferences])}));}
...
export const GENERIC_GEO_V1:VerticalPolicyPackDefinition=Object.freeze({packId:"GENERIC_GEO_V1",verticalSlug:"generic",version:1,effectiveFrom:"2026-07-19T00:00:00.000Z",mandatoryHumanReview:false,rules:Object.freeze([]),evidenceReferences:Object.freeze(["docs/recovery/国内GEO系统冻结结构.md"])});
