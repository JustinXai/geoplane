...
export function ClientAccountWorkspace() {
  ...
  return <AsyncSection state={resource.state} onRetry={resource.reload}>{(data) => {
    ...
    return <div className="cp-stack">
      ...
      <section className="cp-card"><h2>已登记账号</h2>{data.center.accounts.length === 0 ? <p className="cp-list-row cp-list-empty">尚未登记客户账号。</p> : <div className="cp-table-wrap"><table className="cp-data-table"><thead><tr><th>平台与账号</th><th>账号状态</th><th>授权</th><th>凭据状态</th><th>账号健康</th><th>操作方式</th><th>操作</th></tr></thead><tbody>{data.center.accounts.map((account) => <tr key={account.id}><td>{ACCOUNT_PLATFORM_REGISTRY.find((p) => p.code === account.platformCode)?.displayName ?? "未知平台"}<br />{account.displayLabel}</td><td>{accountStatusLabel[account.status]}</td><td>{account.authorizationStatus ? authorizationStatusLabel[account.authorizationStatus] : "待授权"}</td><td>{credentialStatusLabel[account.credentialStatus]}</td><td>{accountHealthLabel[account.healthStatus]}</td><td>{operationModeLabel[account.operationMode]}</td><td>{account.authorizationStatus !== "AUTHORIZED" && account.status === "ACTIVE" ? <button className="cp-button cp-button-primary" type="button" disabled={pendingId !== null} onClick={() => void authorize(account.id, clientOrganizationId)}>{pendingId === account.id ? "授权中…" : "确认授权"}</button> : "—"}</td></tr>)}</tbody></table></div>}
