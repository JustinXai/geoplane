...
export function buildClientOverview(data:ClientOverviewData):ClientOverviewVM{
  ...
  const failedProbeCount=data.probes.filter(item=>item.outcome==="FAILED").length;
  const metrics:ClientOverviewMetricVM[]=[
    ...
    {label:"国内 AI 检测样本",value:String(data.probes.length),source:"来源：人工登记的真实检测样本",href:"#probe-progress"},
  ];
  const risks:string[]=[];
  ...
  if(failedProbeCount>0)risks.push(`国内 AI 检测有 ${failedProbeCount} 条失败样本，需要人工复核。`);
  ...
  const latestProbe=[...data.probes].sort((a,b)=>a.observedAt.localeCompare(b.observedAt)).at(-1);
  if(latestProbe)activities.push({label:"国内 AI 检测登记",detail:latestProbe.outcome==="ANSWERED"?"已获得回答":"检测失败，待复核",occurredAt:latestProbe.observedAt});
